<template>
  <base-modal v-model="isShow">
    <div class="p-8">
      ...
      <div
        class="font-light text-gray-500 mb-4 whitespace-pre-line"
        :class="{ 'text-center': type == 'remove', 'text-center': center }"
      >
        {{ description }}
      </div>
      <div
        class="mt-10 flex justify-end space-x-4"
        :class="{
      ...
      >
        <button
          class="h-10 w-32 bg-red-500 hover:bg-red-600 text-white"
          color="gray"
          @click="isShow = false"
          >Cancel</button
        >
        <button
          class="h-10 w-32 bg-green-500 hover:bg-green-600 text-white"
          color="gray"
          @click="onClick"
